...
Generated by Enhanced 3D Orderbook Visualization System
  ...
  /**
   * Capture and export screenshot of the 3D visualization
   */
  public async exportScreenshot(canvasElement?: HTMLCanvasElement, filename?: string): Promise<void> {
    try {
      // Find the Three.js canvas element if not provided
      const canvas = canvasElement || document.querySelector('canvas') as HTMLCanvasElement;
      ...
      await new Promise(resolve => requestAnimationFrame(resolve)); // Double frame wait for stability
      
      // Convert canvas to blob immediately without modifying canvas size
      return new Promise<void>((resolve, reject) => {
        try {
          // Use canvas.toDataURL as a fallback for better WebGL compatibility
          const dataURL = canvas.toDataURL('image/png', 0.95);
          
          if (!dataURL || dataURL === 'data:,') {
            reject(new Error('Canvas appears to be empty or not properly rendered'));
            return;
          }
          ...
          fetch(dataURL)
            ...
            .then(blob => {
              ...
              const downloadFilename = filename || `orderbook-screenshot-${timestamp}.png`;
  ...
  /**
   * Capture screenshot with custom dimensions and quality
   */
  public async exportHighQualityScreenshot(
    ...
    try {
      ...
      if (!ctx) {
        throw new Error('No valid rendering context found');
      }

      // Use toDataURL for better WebGL compatibility
      const mimeType = format === 'png' ? 'image/png' : 'image/jpeg';
      const dataURL = canvas.toDataURL(mimeType, quality);
      
      if (!dataURL || dataURL === 'data:,' || dataURL.length < 100) {
        throw new Error('Canvas appears to be empty or WebGL context lost. Try again in a moment.');
      }
      ...
      return new Promise<void>((resolve, reject) => {
        fetch(dataURL)
          ...
          .then(blob => {
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
            const downloadFilename = filename || `orderbook-hq-screenshot-${timestamp}.${format}`;
            this.downloadFile(blob, downloadFilename);
            resolve();
          })
          .catch(err => reject(new Error(`Failed to process screenshot: ${err}`)));
      });
    } catch (error) {
      console.error('High quality screenshot capture failed:', error);
      throw error;
    }
  }
}
